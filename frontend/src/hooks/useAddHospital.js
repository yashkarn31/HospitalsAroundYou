...
const useAddHospital = () =>{
    ...
    const add = async ({ name, cityLower, address, imageUrl, specialities, rating, description, numberOfDoctors, numberOfDepartments }) => {
    try{
        ...
        const res = await fetch(`${process.env.BACKEND_URL}/api/v1/hospitals/create`, {
            method: "POST",
            body: JSON.stringify({ name, cityLower, address, imageUrl, specialities, rating, description, numberOfDoctors, numberOfDepartments }),
            headers: {
